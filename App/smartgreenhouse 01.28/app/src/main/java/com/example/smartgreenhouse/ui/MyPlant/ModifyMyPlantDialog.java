...
public class ModifyMyPlantDialog extends Dialog {
    ...
    private String text;
    private int image, location;

    public ModifyMyPlantDialog(Context context, final int position, MyPlant myPlant) {
        super(context);
        requestWindowFeature(Window.FEATURE_NO_TITLE);
        getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
        setContentView(R.layout.modify_my_plant_dialog);

        text = myPlant.getText();
        location = myPlant.getLocation();
        ...
        mod_bt.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (listener != null) {
                    ...
                    MyPlant myPlant = new MyPlant(name, image, location);
