...
public class SignupActivity extends AppCompatActivity
{
    ...
    TextView alarmId, alarmPassword, alarmCheckPassword, alarmName, alarmPhone;
    Button btnRegister, btnCheckDuplicated;
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        alarmId = (TextView) findViewById(R.id.alarmId);

        inputPassword = (EditText) findViewById(R.id.inputPassword);
        alarmPassword = (TextView) findViewById(R.id.alarmPassword);

        checkPassword = (EditText) findViewById(R.id.checkPassword);
        alarmCheckPassword = (TextView) findViewById(R.id.alarmCheckPassword);

        inputName = (EditText) findViewById(R.id.inputName);
        alarmName = (TextView) findViewById(R.id.alarmName);

        inputPhone = (EditText) findViewById(R.id.inputPhone);
        alarmPhone = (TextView) findViewById(R.id.alarmPhone);
            ...
            public void onFocusChange(View v, boolean hasFocus)
            {
                ...
                if (hasFocus == false)
                {
                    if (password.equals("")) // 비밀번호가 입력되지 않았을 시에
                        alarmPassword.setText("비밀번호를 입력해주세요");
                    else
                    {
                        if (password.length() < 8 || password.length() > 16) //비밀번호는 입력됐으나 형식에 안맞을때
                            alarmPassword.setText("비밀번호는 8~16자 이내의 영문과 숫자의 조합으로 이루어져야 합니다");
                    }
                }
                else
                    alarmPassword.setText("");
            }
        });
        ...
        checkPassword.setOnFocusChangeListener(new View.OnFocusChangeListener()
        {
            ...
            public void onFocusChange(View v, boolean hasFocus) {
                ...
                boolean result = comparePw(txtPw, txtCheckPw);
                if (hasFocus == false)
                {
                    if(txtCheckPw.equals(""))
                        alarmCheckPassword.setText("비밀번호 확인을 입력해주세요");
                    else
                    {
                        if (result==false) // 비밀번호 확인은 입력됐으나 비밀번호와 다를 때
                            alarmCheckPassword.setText("비밀번호 확인이 다릅니다");
                    }
                }
                else
                    alarmCheckPassword.setText("");
            }
        });
            ...
            public void onFocusChange(View v, boolean hasFocus)
            {
                ...
                if (hasFocus == false)
                {
                    if (name.equals("")) // 아무것도 입력되지 않았을 시에
                        alarmName.setText("이름을 입력해주세요");
                    else
                    {
                        if(name.length()<2)
                            alarmPhone.setText("이름은 두 글자 이상 입력가능합니다");
                    }
                }
                else
                    alarmName.setText("");
            ...
            public void onFocusChange(View v, boolean hasFocus)
            {
                ...
                if (hasFocus == false)
                {
                    if (phoneNum.equals("")) // 아무것도 입력되지 않았을 시에
                        alarmPhone.setText("휴대전화 번호를 입력해주세요.");
                    else
                    {
                        if(phoneNum.length()<10)
                            alarmPhone.setText("휴대전화 번호가 올바르지 않습니다.");
                    }
                }
                else
                    alarmPhone.setText("");
        ...
        btnRegister.setOnClickListener(new View.OnClickListener()
        {
            ...
            public void onClick(View v)
            {
                ...
                register(txtId, txtPw, txtName, txtPhone); // 데이터베이스에 데이터 전송 및 저장
            }
        });
    ...
    public boolean comparePw(final String password, final String checkPassword)
    {
    ...
    public void register(final String id, final String password, final String name, final String phone)
    {
        ...
        String registerUrl = getString(R.string.registerUrl); //가입하기 jsp
        ...
        }, new Response.ErrorListener()
        {
            ...
            {
                // Error Handling
                Toast.makeText(getApplicationContext(), "시스템 오류 " + error.getMessage(), Toast.LENGTH_SHORT).show();
            }
        })
        {
            @Override
            ...
            public void onResponse(String response)
            {
                ...
                try
                {
                    ...
                    if(result.equals("1"))  // 아이디 중복 O
                        alarmId.setText("이 아이디는 사용하실 수 없습니다");
                    else                    // 아이디 중복 X
                    {
                        isCheckId = true;
