...
public class ItemTouchHelperCallback extends ItemTouchHelper.Callback {
    ...
    public void onChildDraw(@NonNull Canvas c, @NonNull RecyclerView recyclerView, @NonNull RecyclerView.ViewHolder viewHolder, float dX, float dY, int actionState, boolean isCurrentlyActive) {
        ...
        if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE) {
            if (buttonsShowedState != ButtonsState.GONE) {
                if (buttonsShowedState == ButtonsState.UP_VISIBLE) dX = Math.max(dX, buttonWidth);
    ...
    private void drawButtons(Canvas c, RecyclerView.ViewHolder viewHolder) {
        ...
        if (buttonsShowedState == ButtonsState.UP_VISIBLE) {
            RectF upButton = new RectF(itemView.getLeft(), itemView.getTop() - 10, itemView.getTop() + buttonWidthWithOutPadding, itemView.getBottom() - 10);
            ...
            RectF downButton = new RectF(itemView.getLeft() - buttonWidthWithOutPadding, itemView.getTop() + 10, itemView.getBottom() - 10, itemView.getBottom() - 10);
    ...
    private void setTouchListener(final Canvas c, final RecyclerView recyclerView, final RecyclerView.ViewHolder viewHolder, final float dX, final float dY, final int actionState, final boolean isCurrentlyActive) {
        recyclerView.setOnTouchListener(new View.OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                if (swipeBack) {
                    if (dX < -buttonWidth) buttonsShowedState = ButtonsState.DOWN_VISIBLE;
                    else if (dX > buttonWidth) buttonsShowedState = ButtonsState.UP_VISIBLE;
                    if (buttonsShowedState != ButtonsState.GONE) {
