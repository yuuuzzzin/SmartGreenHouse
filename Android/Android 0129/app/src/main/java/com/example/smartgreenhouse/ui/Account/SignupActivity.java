...
public class SignupActivity extends AppCompatActivity
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        phoneMsg = (TextView) findViewById(R.id.alarmPhone);
        ...
        inputId.setFilters(new InputFilter[]{filterEng}); // 아이디 영어만 입력가능
        inputPassword.setFilters(new InputFilter[]{filterEngNum}); // 비밀번호 영어숫자만 입력가능
        checkPassword.setFilters(new InputFilter[]{filterEngNum}); // 비밀번호확인 영어숫자만 입력가능
        ...
        inputPassword.setOnFocusChangeListener(new View.OnFocusChangeListener()
        {
            ...
            public void onFocusChange(View v, boolean hasFocus)
            {
                ...
                if (hasFocus == false)
                {
                    if (password.equals("")) // 비밀번호가 미입력 시
                        passwordMsg.setText("비밀번호를 입력해주세요");
                    else
                    {
                        if (password.length() < 4 || password.length() > 8) //비밀번호 형식에 부적합하면
                           passwordMsg.setText("비밀번호는 4~8자 이내의 영문과 숫자의 조합으로 이루어져야 합니다");
                    }
                }
                else
                    passwordMsg.setText("");
        ...
        inputPhone.setOnFocusChangeListener(new View.OnFocusChangeListener()
        {
            ...
            public void onFocusChange(View v, boolean hasFocus)
            {
                ...
                if (hasFocus == false)
                {
                    if (phoneNum.equals("")) // 휴대전화 번호 미입력시
    ...
    public void register(final String id, final String password, final String name, final String phone)
    {
        ...
        String registerUrl = getString(R.string.signupUrl); //가입하기 jsp
