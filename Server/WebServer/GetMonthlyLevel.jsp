...
//초기 선언
	...
	if(n.equals(id)) {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery("select  substring(date,6,2), avg(level) from sensor where date between '"+strDate+"' and  '"+date+"' group by DATE_FORMAT(date, '%y%m')");
			for(int i=1; i<13; i++){
				if(rs.next()) {
