...
//초기 선언
	...
	if(n.equals(id)) {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery("select  substring(date,7,4), avg(humi) from sensor where date between '"+date+"' and  '"+strDate+"' group by DATE_FORMAT(date, '%y%m%d')");
			for(int i=1; i<8; i++){
				if(rs.next()) {
					jObject.put("RESULT", "1");
					jObject.put("humi"+i, rs.getString("avg(humi)"));
					jObject.put("date"+i, rs.getString("substring(date,7,4)"));
					jArray.add(jObject);

				}
				else{
					jObject.put("humi"+i, "0");
